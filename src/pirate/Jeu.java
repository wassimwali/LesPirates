...
public class Jeu {
	...
	public void lancerJeu() {
		 ...
		 * testtttttttt
		 * 
		 * Carte[] cartes =joueur1.getMain().getMainCarte(); for(int i=0;i < 4 ;i++) {
	...
	private boolean tourJoueur(Pirate joueur, Pirate adversaire) {
		...
		if (!pioche.estVide()) {
			...
			affichage.afficherMessage(joueur.getNom() + " a pioché une carte son nom est: " + cartePiochee.getNom()
					+ " et sa description : " + cartePiochee.getDescription());
		}
		...
		affichage.afficherMessage(joueur.getNom() + " a choisi une carte son nom est: " + carteJouee.getNom()
				+ " et sa description : " + carteJouee.getDescription());
		if (carteJouee != null) {
			...
			} else if (carteJouee instanceof CarteDeffense || carteJouee instanceof CarteDiffamation
					|| carteJouee instanceof CarteChance || carteJouee instanceof CarteSoin) {
				carteJouee.appliquerCarte(joueur);
				carteJouee.afficher();
				ZoneJeu zone = carteJouee.donnerZone();
				affichage.afficherMessage("Et donc il a deposer la carte dans la zone " + zone.toString());
	...
	private boolean partieTerminee() {
		return joueur1.getnbrCoeur() <= 0 || joueur2.getnbrCoeur() <= 0 || joueur1.getPopularite() >= 5
	...
	private void determinerGagnant() {
		if (joueur1.getnbrCoeur() <= 0) {
			affichage
					.afficherMessage(joueur2.getNom() + " a gagné ! " + joueur1.getNom() + " a perdu tout ses coeurs.");
		} else if (joueur2.getnbrCoeur() <= 0) {
			affichage
					.afficherMessage(joueur1.getNom() + " a gagné ! " + joueur2.getNom() + " a perdu tout ses coeurs.");
